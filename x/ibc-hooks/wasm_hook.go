...
import (
	...
	"github.com/aura-nw/aura/x/ibc-hooks/keeper"
	...
	"github.com/aura-nw/aura/x/ibc-hooks/types"
)
...
func NewEmitErrorAcknowledgement(ctx sdk.Context, err error, errorContexts ...string) channeltypes.Acknowledgement {
	...
	err_string := fmt.Sprintf("ABCI code: %d: %s", code, "error handling packet: see events for details")

	return channeltypes.NewErrorAcknowledgement(err_string)
}
